from src.bouquet.utility.printing import add_successfully, del_successfully, invalid_input, separate_line
...
class FlowerInventory:
    ...
    def add_flower(self, menu1):  # function for adding flower to dict...
        ...
        if len(flower_code) != 0 and quantity.isdigit() and len(flower_name) != 0:  # checking whether the quantity is number or not...if yes, then adding to dict...
            if flower_code not in menu1.keys():
                menu1[flower_code] = (quantity, flower_name)
                add_successfully()
            else:
                menu1[flower_code][0] = menu1[flower_code][0] + int(quantity)
                add_successfully()
